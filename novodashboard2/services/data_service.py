        ...
        query_new = """
            SELECT pagePath,
                   ...
                   CASE WHEN COALESCE(SUM(sessions),0) > 0
                        THEN (COALESCE(SUM(userEngagementDuration),0) * 1.0) / NULLIF(COALESCE(SUM(sessions),0),0)
                        ELSE 0.0 END AS avg_engagement_sec
            ...
            return [
                {
                    "page_path": r[0],
                    "page_title": r[1],
                    "pageviews": int(r[2] or 0),
                    "avg_engagement_sec": float(r[3] or 0.0),
                }
                for r in rows
            ]
    except Exception:
...
def get_yt_weekly_comparison(weeks: int = 8) -> List[Dict[str, str]]:
    """Comparação semanal (últimas N) de YouTube: views e minutos.

    Usa `fact_yt_channel_daily` e agrega por semana (YYYY-%W), retornando em ordem crescente.
    """
    con = _ensure_duckdb()
    query = """
        WITH weekly AS (
    ...
    con.close()
    return [
        {"year_week": r[0], "views": int(r[1] or 0), "minutes": int(r[2] or 0)} for r in rows
    ]
