...
def refresh_pages_last_n_days(days: int = 30) -> str:
    ...
    df = df.with_columns(
        [
            ...
            pl.col("userEngagementDuration").cast(pl.Int64, strict=False),
        ]
    )
    if "date" in df.columns:
        df = df.with_columns(
    ...
    # Garantir coluna nova em bases antigas
    try:
        con.execute("ALTER TABLE fact_ga4_pages_daily ADD COLUMN userEngagementDuration BIGINT;")
    except Exception:
        pass
    ...
    try:
        ...
        con.execute(
            ...
            SELECT CAST(date AS DATE), CAST(pagePath AS TEXT), CAST(pageTitle AS TEXT),
                   CAST(screenPageViews AS BIGINT), CAST(sessions AS BIGINT), CAST(totalUsers AS BIGINT), CAST(userEngagementDuration AS BIGINT)
            FROM {tmp_pages};
            """
        )
        con.execute("COMMIT;")
